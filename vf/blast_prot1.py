...
if __name__=='__main__':
  ...
  os.mkdir(fastafl)
  fls=[f for f in os.listdir(cudir+"/dataVF") if f.endswith(".faa")]
  # print fls

  for q,files in enumerate(fls):
     # print files 
     file2=cudir+"/dataVF/"+files
     file3=cudir+"/"+files+"_blast.txt"
     blast_sequences(file1,file2,file3)
     subprocess.call(["python","vftry.py",file1,file2,file3,str(q+1),fastafl],cwd=cudir)
  
  subprocess.call(["python","vfparse_fl.py",file1,fastafl],cwd=cudir)  
 
  with open(cudir+"/"+fastafl+"/"+fastafl+"_vf1_result.txt","w") as finalgut:
