...
#/home/rawal-lab/Desktop/ncbi_tcruzi_species/data/clbrener_seq.fasta
...
def extract_blast_Result(blstfile,file1,file2):
    ...
    df4.to_csv(file2+"Final_max_result.txt",sep="\t",index=False)
    
if __name__=='__main__':
   
    file1=sys.argv[1]
    parse_fasta(file1.strip())
    file2=sys.argv[2]
    parse_fasta(file2.strip())
    filename=str(file1.split("/")).split(".")[0]
    fl1=filename.replace("[\'","")
    filename1=str(file2.split("/")).split(".")[0]
    fl2=filename1.replace("[\'","")
    blstfile=sys.argv[3]
    extract_blast_Result(blstfile,file1,file2)
