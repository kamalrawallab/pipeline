...
if __name__=='__main__':
  ...
  os.mkdir(fastafl)
  fls=[f for f in os.listdir(cudir+"/dataGut") if f.endswith(".faa")]
  for q,files in enumerate(fls):
     #print files 
     file2=cudir+"/dataGut/"+files
     file3=cudir+"/"+files+"_blast.txt"
     blast_sequences(file1,file2,file3)
     subprocess.call(["python","guttry.py",file1,file2,file3,str(q+1)],cwd=cudir)
  subprocess.call(["python","gutparse_fl.py",file1,fastafl],cwd=cudir)  
 
  with open(cudir+"/"+fastafl+"/"+fastafl+"_Gut_flora_result.txt","w") as finalgut:
   fasta_sequence = SeqIO.parse(open(file1),"fasta")
   for fasta in fasta_sequence:
           
        name1, sequence = fasta.id, str(fasta.seq)
        f=0
        with open(cudir+"/"+fastafl+"/"+fastafl+"_gutFlora_final_fl.txt","r") as gutfl:
         for lines in gutfl:
            lines=lines.strip()
            if name1 in lines:
